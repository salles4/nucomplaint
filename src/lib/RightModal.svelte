...
<div class="overflow-hidden absolute print:static w-[100svw] h-[100svh] top-0 left-0 print:h-fit">
  <button
    transition:fade
    class="bg-black/20 absolute w-[100svw] h-[100svh] top-0 left-0 cursor-default z-50 print:hidden"
    on:click={() => closeDetails()}
  >
  </button>
  <div
    transition:fly={{ duration: 500, x: 1000 }}
    class="absolute h-[100svh] print:h-fit print:pb-5 right-0 top-0 bg-white w-full sm:w-[550px] print:w-full rounded-s-2xl z-[60] overflow-y-auto print:static print:overflow-visible flex flex-col"
  >
    ...
    <div class="hidden print:block ">
      <canvas
      id="qrOffense"
      class="w-[120px] mx-auto absolute bottom-0 right-0"
      ></canvas>
    </div>
  </div>
</div>
