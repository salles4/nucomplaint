...
<table>
  ...
  <tr class="hover:bg-black/20 hover:cursor-pointer">
    ...
    <td><button class="text-red-600 underline" on:click={()=>{deleteUser(details.user_id, `${details.first_name} ${details.last_name}` , email)}}>Delete</button></td>
  </tr>
  {/each}
</table>
{:else}
